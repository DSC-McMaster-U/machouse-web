...
import FinancesTable from '../components/Management/FinancesTable';
		...
		<ScrollView style={styles.container}>
			<View style={styles.content}>
				<List.Section title='Pending Tasks:'>
					...
					<List.Accordion title='Shopping List' left={(props) => <List.Icon color='#F3C761' icon='cart-plus' />} id='ShoppingList' style={{ ...styles.card, backgroundColor: colors.accent }} titleStyle={{ color: '#ECECEC', fontSize: 20, fontWeight: 'bold' }}>
						<View style={{ paddingRight: 5, margin: 5 }}>
							<View>
								<TextInput placeholder='Add item...' style={styles.input} onChangeText={(text) => setShoppingListText(text)} />
								<TouchableOpacity style={styles.btn} onPress={() => addShoppingListItem(shoppingListText)}>
					...
					<List.Accordion
						title='Chore List'
						left={(props) => <List.Icon color='#F3C761' icon='clipboard-check-multiple' />}
						id='ChoreList'
						style={{
							...styles.card,
							backgroundColor: colors.accent,
					...
					>
						{
							<View style={{ paddingRight: 5, margin: 5 }}>
								<View>
									<TextInput placeholder='New chore...' style={styles.input} onChangeText={(text) => setChoresText(text)} />
									<TouchableOpacity style={styles.btn} onPress={() => addChore(choresText)}>
