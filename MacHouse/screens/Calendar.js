...
import { SafeAreaView, StyleSheet, Button, Modal, TouchableOpacity, Text, StatusBar } from 'react-native';
import { Agenda } from 'react-native-calendars';
import uuid from 'uuid-random';
import RenderItem from '../components/Calendar/RenderItem';
		...
		<SafeAreaView style={styles.safe}>
			<Modal visible={modalOpen} animationType='slide'>
				<SafeAreaView style={StyleSheet.modalContent}>
					<Ionicons name='close' onPress={() => setMOdalOpen(false)} size={30} style={styles.closeButton}></Ionicons>
					<EventForm items={items} timestamp={timestamp} addItem={addItem} setModalOpen={setMOdalOpen}></EventForm>
				</SafeAreaView>
			</Modal>
			...
			<Agenda
				...
				items={items}
				renderItem={(item) => <RenderItem item={item} items={items} delItem={delItem}></RenderItem>}
